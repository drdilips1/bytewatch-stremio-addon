...
async function yours(genre) {
  const hits = (list) => list.filter((b) => (b.genres || []).some((g) => genre.match.test(g)) || genre.match.test(b.title));
  ...
  const cloudHits = files
    ...
    .filter((b) => b.genres.some((g) => genre.match.test(g)));
  const seen = new Set();
  ...
  return (
    <div class="screen" style={{ '--h': genre.hue }}>
      ...
      <div class="segmented scroll">
        {TABS.filter(([k]) => k !== 'yours' || connected).map(([k, label]) => (
      ...
      {items === null ? (
        ...
        <Empty title="Nothing here yet">
          {tab === 'yours' ? 'None of your books are tagged with this genre yet. Try Bestsellers.' : 'Try another tab or genre.'}
        </Empty>
      )}
