...
final class UtdClient {
    ...
    static final String TOPIC_SCRIPT = "(function(){"
            ...
            + "if(tag==='img'){var src=n.getAttribute('src')||n.getAttribute('data-src')||'';if(!src||/^data:/.test(src))return '';"
            + "if(/logo|wolters|kluwer|brand|sprite|icon|banner|masthead/i.test(src+' '+(n.getAttribute('alt')||''))||n.closest('[class*=logo],[id*=logo],[class*=header],[id*=header],[class*=brand],[class*=banner]'))return '';"
            + "if(n.naturalWidth&&n.naturalWidth<120)return '';try{src=new URL(src,location.href).href;}catch(e){return '';}return '<img src=\"'+esc(src)+'\">';}"
